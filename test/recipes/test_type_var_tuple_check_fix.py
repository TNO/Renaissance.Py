...
class TestFixLegacyUnpackUsage:
    ...
    def test_rewrites_generic_base_unpack_to_star_syntax(
        ...
        # The now-unused `Unpack` import itself is left for ruff's F401 to clean up.
        assert_that(output, contains_string("from typing import TypeVarTuple, Generic, Unpack"))
    ...
    def test_fix_is_written_to_a_real_file_via_run(self, tmp_path: Path) -> None:
        """Regression test: run() must commit(), not just queue the rewrite in memory."""
        ...
        assert_that(subject.result, has_entry("Ts", "fixed"))
        written = target.read_text(encoding="utf-8")
        assert_that(written, contains_string("def foo(*args: *Ts) -> None:"))
        assert_that(written, is_not(contains_string("Unpack[Ts]")))
        assert_that(written, contains_string("from typing import TypeVarTuple, Unpack"))
