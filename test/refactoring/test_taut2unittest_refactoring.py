...
import pytest
from hamcrest import ends_with, assert_that, is_, contains_string
...
class TestTaut2Unittest:
    ...
    def _create_for_filename(self, mocker, filename: Path, text: str = "x = 1") -> Taut2Pyunit:
        code = textwrap.dedent(text)
        mocker.patch(
            "renaissance.impl.python.factory.PythonFactory.create",
            return_value=PythonRstNode.load_from_text(code, str(filename)),
        )
        subject = Taut2Pyunit(str(filename))
        subject.in_memory = True
        return subject

    @pytest.mark.parametrize(
        "source_path, expected_script",
        [
            (
                Path("/repo/alpha/com/int/tst/sample_test.py"),
                "tb_cadenv_exec python alpha/tstpkg/sample_test.py",
            ),
            (
                Path("/repo/com/int/tst/sample_test.py"),
                "tb_cadenv_exec python repo/tstpkg/sample_test.py",
            ),
            (
                Path("/repo/alpha/tests/sample_test.py"),
                "tb_cadenv_exec python sample_test.py",
            ),
        ],
    )
    def test_scenario_exec_script(self, mocker, source_path, expected_script):
        subject = self._create(mocker, "x = 1")
        result = subject._scenario_exec_script(source_path)
        assert_that(result, is_(expected_script))

    def test_file_is_added_to_scenario_when_already_mentioned(self, mocker):
        subject = self._create(mocker, "x = 1")
        mocker.patch.object(subject, "_mentioned_in_scenario", return_value=True)
        new_path = mocker.patch.object(subject, "_new_scenario_file_path")

        subject.add_testfile_to_scenario()

        new_path.assert_not_called()

    def test_file_is_added_to_scenario_creates_xml_for_com_int_tst(self, mocker, tmp_path):
        test_file = tmp_path / "proj_a" / "com" / "int" / "tst" / "sample_test.py"
        ...
        scenario_files = list(test_file.parent.glob("*.test_scenario.xml"))
        assert_that(len(scenario_files), is_(1))
    ...
    <test_case_set>
     <pre_test_case_set_script>system_start -li</pre_test_case_set_script>
      <test_case>
        ...
        <exec_script>tb_cadenv_exec python proj_a/tstpkg/sample_test.py</exec_script>
...
"""
    ...
    def test_file_is_added_to_scenario_creates_xml_with_filename_fallback(self, mocker, tmp_path):
