...
class TestPythonNode:
    ...
    def test_def_call_references(self):
        ...
        assert_that(len(referenced_by), is_(1))  # Function a referenced by function f and var x.
        assert_that(funcDef in [r.node for r in referenced_by])
        ref1 = refs[1]
        ref_node1 = ref1.node
        assert_that(syntax_tree.ASTFinder.matches_kind(ref_node, 'FunctionDef'), is_(True))
        assert_that(ref_node1.name.lower(), is_('b'))
        referenced_by1 = ref_node1.referenced_by
        assert_that(len(referenced_by1), is_(1))  # Function b referenced by function f.
        assert_that(funcDef in [r.node for r in referenced_by])
    ...
    def test_type_reference(self):
        ...
        assert_that(len(refs), is_(1))
        ref = refs[0]
        ref_node = ref.node
        assert_that(syntax_tree.ASTFinder.matches_kind(ref_node, 'Name'), is_(True))
        assert_that(ref_node.name.lower(), is_('a'))
        referenced_by = ref_node.referenced_by
        assert_that(len(referenced_by), greater_than(0))
    ...
    def test_class_reference(self):
        ...
        referenced_by = ref_node.referenced_by
        assert_that(len(referenced_by), is_(2))
    ...
    def test_function_reference(self):
        ...
        assert_that(syntax_tree.ASTFinder.matches_kind(ref_node, 'FunctionDef'), is_(True))
        referenced_by = ref_node.referenced_by
        assert_that(len(referenced_by), is_(1))
