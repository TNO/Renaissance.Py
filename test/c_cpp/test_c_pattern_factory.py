...
class TestCPatternFactory:
    def test_derive_header(self):
        ...
        simple_header = ";\n".join(c.signature for c in atu.children if c.is_part_of_translation_unit()
                                   and not(c.kind == 'FUNCTION_DECL' and c.children[-1].kind =='COMPOUND_STMT'))
...
class TestExpression(TestCPatternFactory):
        ...
        @pytest.mark.parametrize("_, factory, expression, expected",Factories.extend( [
            ('a == $hallo','(BINARY_OPERATOR, , test.c[123:134]): |a == $hallo|\n  (UNEXPOSED_EXPR, a, test.c[123:124]): |a|\n    (DECL_REF_EXPR, a, test.c[123:124]): |a|\n  (_MatchOne__, $hallo, test.c[128:134]): |$hallo|\n    (_MatchOne__, $hallo, test.c[128:134]): |$hallo|\n'),
...
class TestDeclaration(TestCPatternFactory):
        ...
        @pytest.mark.parametrize("_, factory, declarationText, types, parameters, expected_vars, expected_refs",Factories.extend([
            ('int a=3;',[],[],1, 0),   
            ('int a;',[],[],1, 0),   
            ('int a = $x;',[],['$x'],1,1),
...
class TestStatements(TestCPatternFactory):
        ...
        @pytest.mark.parametrize("_, factory, statementText, extra_declarations, expected_stmts, expected_refs",list(Factories.extend( [
            ('a=3;',[],1, 1),   
            ('a = b;',[],1, 2),   
            ('a = $x;',[],1,2),
            ('a=2;b = 3;c=4;',[],3,3),
...
class TestUseAtuToCreatePatterns(TestCPatternFactory):
    ...
    @pytest.mark.parametrize("_, factory, statementText, expected_stmts, expected_refs",list(Factories.extend( [
        ('A a = {};',1, 1),
