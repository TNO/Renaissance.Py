...
class TestExpressions(TestCMatchFinder):
    def test_match_expr(self):
        ...
        @pytest.mark.parametrize("_, factory, expression, expected_full_matches, expected_dicts_per_match",Factories.extend([
        ('a == 3',['a==3'], [{}]),
        ('a == $x',['a==3', 'a==4'], [{'$x':['3']},{'$x':['4']}]),
...
class TestUseAtuToCreatePattern(TestCMatchFinder):
        ...
        def test(self, _, factory, statements, pattern_type, expected, names):
            ...
            assert_that(result.to_list(), has_length(greater_than(1)))  # should find multiple matches, at least the one in the pattern and the one in the function body
