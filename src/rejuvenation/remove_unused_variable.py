...
def remove_unused_variable_low_level(node_type: type[ASTNode]):
    ...
    ASTFinder.find_kind(atu, "(?i)Compound?Stmt").flat_map(lambda func: ASTFinder.find_kind(func, "(?i)Var_?Decl")).filter(
        lambda node: len(node.referenced_by) == 0
    ).map(lambda node: node.parent).for_each(lambda node: rewriter.remove(node, True, True))
