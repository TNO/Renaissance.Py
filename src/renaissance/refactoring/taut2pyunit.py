...
def add_self(ast_refactor):
    ...
    """
    matching = ['emrwxread', 'emrwxwidxread', 'emrwxviprxinterface', 'whxstream2', 'gtaaxtxmark', 'mark_upd_q']
    ...
    ast_refactor.find_kind('Name'). \
        ...
        for_each(lambda node: ast_refactor.replace('self.' + node.name, node, False, False))

def remove_decorator(ast_refactor):
    ast_refactor.find_kind('Attribute'). \
        filter(lambda node: node.name == 'TAUT.log_stub'). \
        for_each(lambda node: ast_refactor.remove(node, False, False))

def convert_assert(ast_refactor):
    ast_refactor.find_kind('Attribute'). \
        filter(lambda node: node.name == 'self.assert_equal'). \
        for_each(lambda node: ast_refactor.replace('self.assertEqual', node, False, False))
...
def replace_taut(ast_refactor):
    ...
    """
    ast_refactor.find_kind('Attribute'). \
        filter(lambda node: node.name == 'TAUT.TestCase'). \
        for_each(lambda node: ast_refactor.replace('unittest.TestCase', node, False, False))
    ast_refactor.find_kind('Name'). \
        filter(lambda node: node.name == 'TestCase'). \
        for_each(lambda node: ast_refactor.replace('unittest.TestCase', node, False, False))
