...
class Taut2Pyunit(PythonRefactoring):
    ...
    def add_testfile_to_scenario(self):
        """
        If this testfile is never mentioned in any of this directory's scenario files: 
        make a new scenario file and execute it there.
        """
        if self._mentioned_in_scenario():
            return

        source_path = Path(self.filename)
        scenario_path = self._new_scenario_file_path(source_path)
        exec_script = self._scenario_exec_script(source_path)

        scenario_content = f"""<?xml version=\"1.0\" encoding=\"UTF-8\"?>
<scenario>
  <configuration>
    <configuration_id>Dummy Configuration id generated by Reborn</configuration_id>
    <environment>DEVBENCH</environment>
    <test_case_set>
     <pre_test_case_set_script>system_start -li</pre_test_case_set_script>
      <test_case>
        <test_case_id>Dummy test case id generated by Reborn</test_case_id>
        <exec_script>{exec_script}</exec_script>
...
"""
    ...
    def _mentioned_in_scenario(self):
        ...
        for scenario_file in scenario_files:
            try:
                contents = scenario_file.read_text(encoding="utf-8", errors="ignore")
            except OSError:
                continue
    ...
    def _new_scenario_file_path(self, source_path: Path) -> Path:
        ...
        suffix = 1
        while scenario_path.exists():
            scenario_path = source_path.parent / f"{source_path.stem}_{suffix}.test_scenario.xml"
            suffix += 1
        return scenario_path

    def _scenario_exec_script(self, source_path: Path) -> str:
        parts = source_path.parent.parts

        # Extract /com/int/tst from filepath, and get its parent folder
        for index in range(len(parts) - 2):
            if parts[index] == "com" and parts[index + 1] == "int" and parts[index + 2] == "tst":
                if index > 0:
                    root = parts[index - 1]
...
"""
    ...
    def with_testdoubles(self):
        pattern = self.pattern_factory.create_expression("TAUT.TestDoubles(module=$a, $b=$c)")
        for match in match_pattern(self.root.children, [pattern]):
            repl = f"patch.object({match['$a']}, '{match['$b']}', {match['$c']})"
            self.replace(repl, match.nodes, False, False)
