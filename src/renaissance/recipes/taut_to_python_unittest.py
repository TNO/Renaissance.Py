...
class TautToPythonUnittest(PythonRefactoring):
    ...
    def replace_taut(self):
        ...
        [
            self.replace("unittest.TestCase", node, False, False)
            for node in self.find_semantic_kind(SemanticKind.NAME)
    ...
    def convert_assert(self):
        ...
        [
            ...
            for node in self.find_semantic_kind(SemanticKind.ATTRIBUTE)
            if node.name == "self.assert_false"
        ]
        [
            self.replace("self.assertTrue", node, False, False)
            for node in self.find_semantic_kind(SemanticKind.ATTRIBUTE)
            if node.name == "self.assert_true"
        ]
        [
...
"""
        ...
        for match in match_pattern(self.root.children, tds_pattern):
            ...
            for matched_doubles in match_pattern(match.expansions["$$aa"], [doubles_pattern]):
                ...
                repl += (
                    f"    patch.object({interface_stub}, '{matched_doubles.expansions['$a'][0]}', "
                    f"self.{matched_doubles.expansions['$a'][0]}),\n"
                )
            repl += "]\n\n"
...
"""
    ...
    def find_import_interface(self, name: str):
        ...
        if name.islower():
            ...
            if node_list:
                if node_list[0].parser_kind == "ImportFrom":
    ...
    def convert_setup(self):
        ...
        for setup_func in match_pattern(self.root.children, pattern_outer):
            ...
            for index, match in enumerate(matched_pattern):
                repl_pattern = (
                    f"self.patches.append(patch.object({match.expansions['$mod'][0].name}, "
                    f"'{match.expansions['$b'][0]}', {match.expansions['$c'][0].signature}))"
                )
