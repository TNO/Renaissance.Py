...
@pytest.mark.parametrize("code, pattern",[
        ("int $body=0;int main() { return 0; }", "int $body=0;int main() { return $body; }"),
        ("int $init, $cond, $inc=0;int $body=0;for (;;) {}", "int $init, $cond, $inc=0;int $body=0;for ($init; $cond; $inc) $body"),
        ("a = b;", "$lhs = $rhs;"),
        ...
        ("int $E=0; int $vals=0; enum E { A };", "int $E=0; int $vals=0;enum $E { $vals };"),
...
@pytest.mark.parametrize("code, pattern",[
        ...
        ("int add(int a, int b) { return a + b; }", "int $a,$b,$body;int $f(int $a, int $b) { $body; }"),
...
from syntax_tree.match_finder import is_match, is_match_tree, MatchFinder
