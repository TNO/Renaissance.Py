...
class CPatternFactory(Generic[ASTNodeType]):
    ...
    def _create_body(self, text, types, parameters, extra_declarations,kind:str):
        fullText = \
            self.header+\
        ...
        return Stream(ASTFinder.find_kind(root.get_children()[-1], '(?i)COMPOUND_?STMT').find_first().get().get_children()).\
            filter(ASTNode.is_part_of_translation_unit).\
